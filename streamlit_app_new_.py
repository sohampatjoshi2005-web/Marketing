...
def patch_lead_profile(lead_id, payload):
    ...
    if result is not None:
        # Invalidate caches for this lead after a successful patch
...
def build_row(profile: dict) -> dict:
    if not profile or not isinstance(profile, dict):
        profile = {}
    row = dict(profile)
    # customer_tier: derive from segment only if backend didn't supply it
    if "customer_tier" not in row:
        row["customer_tier"] = row.get("segment")  # may be None — UI will show placeholder
    # numeric fields: keep None when backend omits them so UI can show "N/A" placeholders
    ...
    # string/enum fields: keep None so UI can distinguish "missing" from a real value
...
st.sidebar.markdown("Navigate through the 9-step architecture.")

nav_options = [
    "1. Customer Engagement Tracking",
    "2. Customer Identity Resolution",
    "3. Customer 360 Profile",
    "4. Customer Intelligence & Scoring",
    "5. Intelligent Customer Segmentation",
    "6. Campaign Orchestration Hub",
    "7. Channel & Content Execution",
    "8. CRM & Analytics Dashboard",
    "9. Recommended Next Best Action",
]
selection = st.sidebar.radio("Go to step:", nav_options)

# -----------------------------------------------------------------------------
# STEP 1 — Customer Engagement Tracking
# -----------------------------------------------------------------------------
if selection == "1. Customer Engagement Tracking":
    st.header("Step 1: Customer Engagement Tracking")
    ...
    with st.form("event_track_form"):
        ...
        with col1:
            u_email = st.text_input("Customer Identity (Email)",
    ...
    if submit_event:
        if not valid_email(u_email):
            ...
            if result:
                st.success(f"Event logged! Lead ID: {result.get('lead_id')} · Segment: {result.get('segment', '—')}")
...
# -----------------------------------------------------------------------------
elif selection == "7. Channel & Content Execution":
    st.header("Step 7: Channel & Content Execution")
        ...
        with tab1:
            ...
            if st.button("Draft Email via HF API"):
                with st.spinner("Drafting Email..."):
                    ...
                    if lead_id and st.button("Send Email Campaign", key="send_email"):
        ...
        with tab2:
            ...
            if st.button("Draft SMS via HF API"):
                with st.spinner("Drafting SMS..."):
                    ...
                    if lead_id and st.button("Send SMS Campaign", key="send_sms"):
...
# -----------------------------------------------------------------------------
# STEP 6 — Campaign Orchestration Hub
# -----------------------------------------------------------------------------
elif selection == "6. Campaign Orchestration Hub":
    st.header("Step 6: Campaign Orchestration Hub")
...
# -----------------------------------------------------------------------------
# STEP 8 — CRM & Analytics Dashboard
# -----------------------------------------------------------------------------
elif selection == "8. CRM & Analytics Dashboard":
    st.header("Step 8: CRM & Analytics Dashboard")
    ...
    if not leads:
        ...
        with col2:
            ...
            if events_raw:
                try:
                    ev_df = pd.DataFrame([
                        {"Action": e.get("action_type", ""), "Date": humanize_ts(e.get("created_at", ""))}
...
# -----------------------------------------------------------------------------
    ...
    _step_label = {
        "2. Customer Identity Resolution":      ("Step 2: Customer Identity Resolution",      "Real-time identity stitching & profile resolution"),
        "3. Customer 360 Profile":              ("Step 3: Customer 360 Profile",              "Unified customer profile — preferences, consent, journey & engagement"),
        "4. Customer Intelligence & Scoring":   ("Step 4: Customer Intelligence & Scoring",   "Scoring & predictive intelligence dashboard"),
        "5. Intelligent Customer Segmentation": ("Step 5: Intelligent Customer Segmentation", "Tier & Lifecycle segmentation across all customers"),
    }[selection]
    ...
    if selection != "5. Intelligent Customer Segmentation":
        st.markdown("""
        <div style="background:#12122a;border:1px solid #2a2a4a;border-radius:10px;padding:16px 20px;margin-bottom:20px;">
            <div style="font-size:12px;font-weight:700;letter-spacing:1.5px;color:#666;margin-bottom:10px;">🔍 LEAD SELECTOR</div>
        """, unsafe_allow_html=True)
        search_query = st.text_input("Search by email, name, or company", "", label_visibility="collapsed",
        ...
        lead_labels = filtered_df.apply(
            lambda r: f"{r['email']}  [{r['name'] if r['name'] else '—'}]", axis=1
        ).tolist()
    ...
    if row is not None:
        ...
        # ── Helper: render recent events table ───────────────────────────────
        ...
        # Real-time identity stitching & profile resolution
        # =====================================================================
        if selection == "2. Customer Identity Resolution":
 
            st.markdown("<div style='margin-top:4px;'></div>", unsafe_allow_html=True)
 
            # ── Identity Summary ─────────────────────────────────────────────
            name_val    = row.get("name") or "—"
            company_val = row.get("company") or "—"
            title_val   = row.get("title") or "—"
            mobile_val  = row.get("mobile_number") or "—"
            member_val  = row.get("membership_id") or "—"
            consent_val = str(row.get("consent_status") or "pending")
            if consent_val.lower() not in ("granted", "withdrawn", "pending"):
                consent_val = "pending"
            consent_color = {"granted": "#34d399", "pending": "#fbbf24", "withdrawn": "#f87171"}.get(consent_val.lower(), "#fbbf24")
            is_dup = bool(row.get("is_duplicate", False))
            dup_badge = ('<span style="background:#3d1a1a;color:#f87171;border:1px solid #f87171;'
            ...
            with id_col:
                st.markdown(f"""
                <div style="background:#1e1e2e;border:1px solid #2a2a4a;border-radius:12px;padding:22px 26px;height:100%;">
                    ...
                    <table style="width:100%;border-collapse:collapse;font-size:13px;color:#ccc;">
                        <tr><td style="padding:6px 0;color:#666;width:40%;">📧 Email</td><td style="padding:6px 0;font-weight:500;">{row.get('email','')}</td></tr>
                        <tr><td style="padding:6px 0;color:#666;">📱 Mobile</td><td style="padding:6px 0;">{mobile_val}</td></tr>
                        <tr><td style="padding:6px 0;color:#666;">🪙 Membership ID</td><td style="padding:6px 0;">{member_val}</td></tr>
                        <tr><td style="padding:6px 0;color:#666;">✅ Consent</td>
                            <td style="padding:6px 0;"><span style="color:{consent_color};font-weight:700;">{consent_val.upper()}</span></td></tr>
            ...
            with meta_col:
                ...
                _conf_label = f"{conf_icon} {_conf_disp}"
                dup_text  = "⚠ Duplicate record detected for this identity." if is_dup else "✓ No duplicate records found."
                ...
                <div style="display:flex;flex-direction:column;gap:14px;">
                    ...
                    <div style="background:#1e1e2e;border:1px solid #2a2a4a;border-radius:12px;padding:18px 22px;">
                        ...
                        <div style="background:#2a2a3a;border-radius:6px;height:8px;overflow:hidden;">
                            <div style="width:{'100%' if conf_val=='high' else '55%' if conf_val=='medium' else '25%' if conf_val=='low' else '0%'};height:100%;background:{conf_css_color};border-radius:6px;"></div>
                        </div>
            ...
            # ── Identity Signals ─────────────────────────────────────────────
            st.markdown("""
            <div style="font-size:12px;font-weight:700;letter-spacing:1.5px;color:#666;margin-bottom:8px;margin-top:4px;">📡 IDENTITY SIGNALS</div>
            """, unsafe_allow_html=True)
        ...
        # =====================================================================
        # STEP 3 — Customer 360 Profile
        # Unified customer profile
        # =====================================================================
        elif selection == "3. Customer 360 Profile":
            ...
            with left_col:
                ...
                consent_color_id = {"granted": "#34d399", "pending": "#fbbf24", "withdrawn": "#f87171"}.get(consent_val_raw.lower(), "#fbbf24")
                is_dup = bool(row.get("is_duplicate", False))
                dup_badge = ('<span style="background:#3d1a1a;color:#f87171;border:1px solid #f87171;'
                             'border-radius:10px;padding:2px 9px;font-size:11px;font-weight:700;">⚠ DUPLICATE</span>'
                             if is_dup else
                ...
                st.markdown(f"""
                <div style="background:#1e1e2e;border:1px solid #2a2a4a;border-radius:12px;padding:22px 26px;">
                    <div style="font-size:12px;font-weight:700;letter-spacing:1.5px;color:#666;margin-bottom:14px;">🪪 IDENTITY SUMMARY</div>
                    <div style="margin-bottom:14px;">
                    ...
                    <table style="width:100%;border-collapse:collapse;font-size:13px;color:#ccc;">
                        <tr><td style="padding:6px 0;color:#666;width:42%;">📧 Email</td><td style="padding:6px 0;font-weight:500;">{row.get('email','')}</td></tr>
                        <tr><td style="padding:6px 0;color:#666;">📱 Mobile</td><td style="padding:6px 0;">{mobile_val}</td></tr>
                        <tr><td style="padding:6px 0;color:#666;">🪙 Membership ID</td><td style="padding:6px 0;">{member_val}</td></tr>
                        <tr><td style="padding:6px 0;color:#666;">🔁 Duplicate</td>
                            <td style="padding:6px 0;">{dup_badge}</td></tr>
            ...
            with right_col:
                ...
                fav_cat      = str(row.get("favourite_category") or "—")
                _journey_raw = row.get("customer_journey")
                journey      = str(_journey_raw) if _journey_raw else "—"
                last_seen    = humanize_ts(row.get("last_interaction", ""))
                engagement   = str(row.get("engagement_summary") or "—")
                notes_val    = str(row.get("customer_notes") or "—")
 
                _consent_raw3 = row.get("consent_status")
                consent_disp3 = str(_consent_raw3).upper() if _consent_raw3 else "PENDING"
                consent_color3 = {"GRANTED": "#34d399", "PENDING": "#fbbf24", "WITHDRAWN": "#f87171"}.get(consent_disp3, "#fbbf24")
 
                pref_ch_raw = row.get("preferred_channel")
                pref_ch_single = str(pref_ch_raw).upper() if pref_ch_raw else "—"
                _pch_tc, _pch_bg = CHANNEL_COLORS.get(str(pref_ch_raw or "").lower(), ("#aaa", "#2a2a3a"))
                pref_ch_pill = f'<span style="background:{_pch_bg};color:{_pch_tc};border:1px solid {_pch_tc};border-radius:16px;padding:3px 11px;font-size:12px;">{pref_ch_single}</span>' if pref_ch_raw else '<span style="color:#555;">—</span>'
 
                pills_html = ""
                if channel_list:
                    for ch in channel_list:
                        tc, bg = CHANNEL_COLORS.get(ch.lower(), ("#aaa", "#2a2a3a"))
                        pills_html += f'<span style="background:{bg};color:{tc};border:1px solid {tc};border-radius:16px;padding:3px 11px;font-size:12px;margin-right:5px;margin-bottom:4px;display:inline-block;">{ch}</span>'
                else:
                    pills_html = '<span style="color:#555;">—</span>'
 
                _jc, _jbg = JOURNEY_COLORS.get(journey.lower(), ("#aaa", "#2a2a3a")) if _journey_raw else ("#555", "#1e1e2e")
                journey_pill = f'<span style="background:{_jbg};color:{_jc};border:1px solid {_jc};border-radius:16px;padding:3px 11px;font-size:12px;">{journey.upper() if _journey_raw else "—"}</span>'
        ...
        # Scoring + predictive intelligence dashboard
        # =====================================================================
        elif selection == "4. Customer Intelligence & Scoring":
 
            # ── TOP: KPI Strip ────────────────────────────────────────────────
            st.markdown("""
            <div style="font-size:12px;font-weight:700;letter-spacing:1.5px;color:#666;margin-bottom:10px;">📊 KEY PERFORMANCE INDICATORS</div>
            """, unsafe_allow_html=True)
            ...
            # ── MIDDLE: Customer Intelligence (left) + Commercial Metrics (right) ──
            intel_col, comm_col = st.columns(2)
 
            with intel_col:
                _tier_disp  = f"{tier_badge_icon} {tier_val.upper()}" if tier_val else "⚪ N/A"
                _lc_disp    = f"{lc_ic} {lc.upper()}" if lc else "⚪ N/A"
                _conf_disp  = f"{conf_icon} {conf_val.upper()}" if conf_val else "⚪ N/A"
                _churn_pct  = churn if churn is not None else 0
                _churn_label = f"{churn}%" if churn is not None else "N/A"
                _urgency_pct = urgency if urgency is not None else 0
                _urgency_label = f"{urgency}/100" if urgency is not None else "N/A"
                ...
                <div style="background:#1e1e2e;border:1px solid #2a2a4a;border-radius:12px;padding:22px 26px;">
                    ...
                    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:18px;">
                        <span style="background:{tier_bg};color:{tier_badge_color};border:1px solid {tier_badge_color};
                            border-radius:20px;padding:5px 14px;font-size:13px;font-weight:700;">{_tier_disp}</span>
                        <span style="background:{lc_bg};color:{lc_text_color};border:1px solid {lc_text_color};
                            border-radius:20px;padding:5px 14px;font-size:13px;font-weight:700;">{_lc_disp}</span>
                    </div>
            ...
            with comm_col:
                ...
                def _rfm_bar(val, color):
                    ...
                    return f'<div style="background:#2a2a3a;border-radius:4px;height:8px;"><div style="width:{pct:.0f}%;height:100%;background:{color};border-radius:4px;"></div></div>'
                ...
                <div style="background:#1e1e2e;border:1px solid #2a2a4a;border-radius:12px;padding:22px 26px;">
                    ...
                    <div style="margin-bottom:10px;">
                        <div style="display:flex;justify-content:space-between;font-size:12px;color:#aaa;margin-bottom:4px;">
                            <span>Recency</span><span style="color:#a78bfa;font-weight:700;">{"N/A" if rfm_r_v is None else rfm_r_v}</span>
                        </div>
                        {_rfm_bar(rfm_r_v, '#a78bfa')}
                    </div>
                    <div style="margin-bottom:10px;">
                        <div style="display:flex;justify-content:space-between;font-size:12px;color:#aaa;margin-bottom:4px;">
                            <span>Frequency</span><span style="color:#34d399;font-weight:700;">{"N/A" if rfm_f_v is None else rfm_f_v}</span>
                        </div>
                        {_rfm_bar(rfm_f_v, '#34d399')}
                    </div>
                    <div>
                        <div style="display:flex;justify-content:space-between;font-size:12px;color:#aaa;margin-bottom:4px;">
                            <span>Monetary</span><span style="color:#60a5fa;font-weight:700;">{"N/A" if rfm_m_v is None else rfm_m_v}</span>
                        </div>
                        {_rfm_bar(rfm_m_v, '#60a5fa')}
                    </div>
                </div>
                """, unsafe_allow_html=True)
            ...
            # ── BOTTOM: Next Best Action ─────────────────────────────────────
            st.markdown("""
            <div style="font-size:12px;font-weight:700;letter-spacing:1.5px;color:#666;margin-bottom:8px;">🚀 NEXT BEST ACTION</div>
            """, unsafe_allow_html=True)
    ...
    # ── Segmentation Table (Step 5 only) ──────────────────────────────────────
    if selection == "5. Intelligent Customer Segmentation":
        st.subheader("📋 Customer Segmentation Table")
        # ── Segmentation Filters ────────────────────────────────────────────
        fc1, fc2, fc3 = st.columns(3)
        with fc1:
            _tier_opts = ["All"] + sorted(full_df["customer_tier"].dropna().unique().tolist()) if "customer_tier" in full_df.columns else ["All"]
            f_tier = st.selectbox("Filter by Customer Tier", _tier_opts, key="seg_filter_tier")
        with fc2:
            _lc_opts = ["All"] + sorted(full_df["lifecycle_stage"].dropna().unique().tolist()) if "lifecycle_stage" in full_df.columns else ["All"]
            f_lc = st.selectbox("Filter by Lifecycle Stage", _lc_opts, key="seg_filter_lc")
        with fc3:
            f_churn = st.selectbox("Filter by Churn Risk", ["All", "Low (<40)", "Medium (40-69)", "High (≥70)"], key="seg_filter_churn")
        ...
        # Apply segmentation filters (Step 5 only)
        if selection == "5. Intelligent Customer Segmentation":
        ...
        if "clv" in seg_df.columns:
            seg_df["clv"] = seg_df["clv"].apply(lambda x: round(float(x), 1) if x is not None else None)
        if "churn_risk_score" in seg_df.columns:
            seg_df["churn_risk_score"] = seg_df["churn_risk_score"].apply(lambda x: int(x) if x is not None else None)
        if "rfm_total_score" in seg_df.columns:
            seg_df["rfm_total_score"] = seg_df["rfm_total_score"].apply(lambda x: int(x) if x is not None else None)
...
# -----------------------------------------------------------------------------  
# -----------------------------------------------------------------------------
# STEP 9 — Recommended Next Best Action
# -----------------------------------------------------------------------------
elif selection == "9. Recommended Next Best Action":
    st.header("Step 9: Recommended Next Best Action")
    ...
    if not leads_raw:
        ...
        if not rows:
            ...
            # Try to get richer data from profile endpoint
            if lead_id:
                with st.spinner("Loading profile..."):
            ...
            with col2:
                ...
                summary["churn_risk_score"] = pd.to_numeric(summary["churn_risk_score"], errors="coerce").apply(lambda x: int(x) if x is not None else None)
