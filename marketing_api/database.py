...
def init_db():
    ...
    c.execute('CREATE UNIQUE INDEX IF NOT EXISTS idx_leads_ucp_id ON leads(ucp_id)')
...
def _generate_ucp_id(conn):
    """Generate next sequential UCP-XXXX id."""
    c = conn.cursor()
    c.execute("SELECT ucp_id FROM leads WHERE ucp_id IS NOT NULL ORDER BY id DESC LIMIT 1")
    row = c.fetchone()
    if row and row["ucp_id"]:
        try:
            last_num = int(row["ucp_id"].split("-")[1])
        except Exception:
            last_num = 1000
    else:
        last_num = 1000
    return f"UCP-{last_num + 1}"
...
def _probabilistic_duplicate_check(lead_id, conn):
    """
    Compare lead against all non-duplicate leads using weighted field similarity.
    Weights: name(30), company(25), title(15), preferred_channel(15), engagement_summary(15).
    Returns (canonical_lead_id, score, confidence) or (None, 0, None).
    score > 70  → probable duplicate → confidence = 'medium'
    score 40-70 → possible duplicate → confidence = 'low'  (no duplicate flag set)
    below 40    → unique
    """
    c = conn.cursor()
    c.execute(
        """SELECT l.id, p.name, p.company, p.title, p.preferred_channel, p.engagement_summary
           FROM leads l
           LEFT JOIN customer_profiles p ON l.id = p.lead_id
           WHERE l.id != ? AND (l.is_duplicate = 0 OR l.is_duplicate IS NULL)""",
        (lead_id,)
    )
    ...
    # Fetch current lead's profile fields
    c.execute(
        """SELECT p.name, p.company, p.title, p.preferred_channel, p.engagement_summary
           FROM customer_profiles p WHERE p.lead_id = ?""",
        (lead_id,)
    )
    me = c.fetchone()
    if not me:
    ...
    for candidate in candidates:
        raw = (
            _similarity(me['name'], candidate['name']) * WEIGHTS['name'] +
            _similarity(me['company'], candidate['company']) * WEIGHTS['company'] +
            _similarity(me['title'], candidate['title']) * WEIGHTS['title'] +
            _similarity(me['preferred_channel'], candidate['preferred_channel']) * WEIGHTS['preferred_channel'] +
            _similarity(me['engagement_summary'], candidate['engagement_summary']) * WEIGHTS['engagement_summary']
        )
        if raw > best_score:
            best_score = raw
    ...
    if best_score > 70:
        return best_id, best_score, 'medium'
    elif best_score >= 40:
        return best_id, best_score, 'low'
...
def _prune_identity_signals(lead_id, conn):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT email, mobile_number, membership_id, ucp_id FROM leads WHERE id = ?",
        (lead_id,),
    )
    lead = c.fetchone()
    preferred = {}
    if lead:
        for col in ("email", "mobile_number", "membership_id", "ucp_id"):
            if lead[col]:
...
def log_event(email, action_type, metadata_dict, visitor_id=None,
              membership_id=None, mobile_number=None, ucp_id=None):
    conn = get_db_connection()
    c = conn.cursor()

    # --- Deterministic identity resolution: email > mobile_number > ucp_id ---
    lead_id = None
    ...
    if lead_id is None and ucp_id:
        c.execute("SELECT id FROM leads WHERE ucp_id = ?", (ucp_id,))
        row = c.fetchone()
        if row:
            lead_id = row['id']
            if email:
    ...
    # --- Exact-match duplicate detection (email, mobile_number, ucp_id) ---
    canonical_lead_id = None
    deterministic_match = False
    if lead_id is not None:
        # Check if another lead already owns one of the exact same identifiers
        for field, val in [('email', email), ('mobile_number', mobile_number), ('ucp_id', ucp_id)]:
            if not val:
                continue
            c.execute(f"SELECT id FROM leads WHERE {field} = ? AND id != ?", (val, lead_id))
            match = c.fetchone()
            if match:
                canonical_lead_id = match['id']
                deterministic_match = True
                c.execute(
                    "UPDATE leads SET is_duplicate = 1, identity_confidence = 'high', canonical_lead_id = ? WHERE id = ?",
                    (canonical_lead_id, lead_id)
                )
                break

    if lead_id is None:
        # New lead — auto-generate ucp_id
        new_ucp_id = ucp_id or _generate_ucp_id(conn)
        c.execute(
            "INSERT INTO leads (email, score, segment, membership_id, mobile_number, ucp_id, identity_confidence, created_at) "
            "VALUES (?, 1, 'bronze', ?, ?, ?, 'low', ?)",
            (email, membership_id, mobile_number, new_ucp_id, datetime.now().isoformat())
        )
        lead_id = c.lastrowid
        c.execute("INSERT INTO customer_profiles (lead_id) VALUES (?)", (lead_id,))
        conn.commit()  # flush so probabilistic query can see the new profile row
        # Probabilistic check for newly created lead
        prob_canonical_id, prob_score, prob_confidence = _probabilistic_duplicate_check(lead_id, conn)
        if prob_confidence == 'medium':
            c.execute(
                "UPDATE leads SET is_duplicate = 1, identity_confidence = 'medium', canonical_lead_id = ? WHERE id = ?",
                (prob_canonical_id, lead_id)
            )
        elif prob_confidence == 'low':
            # Possible duplicate — flag confidence only, do not mark as duplicate
            c.execute(
                "UPDATE leads SET identity_confidence = 'low' WHERE id = ?",
                (lead_id,)
            )
    else:
        ...
        if ucp_id:
            c.execute("UPDATE leads SET ucp_id = ? WHERE id = ? AND ucp_id IS NULL", (ucp_id, lead_id))
        # Ensure existing leads without a ucp_id get one assigned
        c.execute("SELECT ucp_id FROM leads WHERE id = ?", (lead_id,))
        existing_ucp = c.fetchone()
        if existing_ucp and not existing_ucp['ucp_id']:
            c.execute("UPDATE leads SET ucp_id = ? WHERE id = ?", (_generate_ucp_id(conn), lead_id))
        # Probabilistic check only if deterministic match did not already resolve identity
        if not deterministic_match:
            ...
            if prob_confidence == 'medium':
                ...
                c.execute(
                    "UPDATE leads SET identity_confidence = 'low' WHERE id = ? AND identity_confidence NOT IN ('high', 'medium')",
                    (lead_id,)
                )
    ...
    if ucp_id:
        _upsert_identity_signal(lead_id, 'ucp_id', ucp_id, conn)
...
def update_profile(lead_id, profile_dict):
    ...
    ucp_id = profile_dict.get('ucp_id')
    if membership_id:
    ...
    if ucp_id:
        c.execute("UPDATE leads SET ucp_id = ? WHERE id = ? AND ucp_id IS NULL", (ucp_id, lead_id))
        _upsert_identity_signal(lead_id, 'ucp_id', ucp_id, conn)
    if membership_id or mobile_number or ucp_id:
        _compute_identity_confidence(lead_id, conn)
    conn.commit()
    conn.close()
